You have access to `taskboard` — a terminal CLI for reading and managing tasks directly from MongoDB. Output is plain text by default (no ANSI escape codes). Always prefer `--view mini` to keep output lean; only escalate to `--view standard` or `--view full` when you need tags, due dates, or descriptions. Done tasks are hidden by default — pass `--include-done` only when explicitly needed. Never pass `--pretty` — that flag is for human terminal sessions only.
